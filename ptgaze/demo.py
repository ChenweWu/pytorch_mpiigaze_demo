...
class Demo:
    ...
    def _process_image(self, i,image) -> None:
    ...
    def _draw_gaze_vector(self, face: Face) -> None:
        ...
        df=pd.DataFrame(columns=['vector','yaw','pitch'])
        if self.config.mode == 'MPIIGaze':
            for key in [FacePartsName.REYE, FacePartsName.LEYE]:
                ...
                logger.info(f'{eye.gaze_vector}')
                df=df.append({'vector' : eye.gaze_vector, 'pitch' : pitch, 'yaw' : yaw},ignore_index=True)
            return df
        elif self.config.mode in ['MPIIFaceGaze', 'ETH-XGaze']:
            self.visualizer.draw_3d_line(
            ...
            logger.info(f'printing:{face.gaze_vector}')
            df=df.append({'vector' : face.gaze_vector, 'pitch' : pitch, 'yaw' : yaw},ignore_index=True)
            return df
        else:
            raise ValueError
